        ...
        <div class="menu-list" style="color: white">
          <ul>
            <li v-for="fav in favorites" :key="fav._id">
              <a :href="/store/ + fav.store._id"> {{ fav.store.storeName }}</a>
            </li>
          </ul>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async getFavorite() {
      ...
      const get_favorites = await fetch(
        `http://localhost:3001/favorite/all?user_id=${userData._id}`,
        {
          method: "GET",
          credentials: "include",
          headers: {
